...
import { remnaCreateUser, remnaUpdateUser, isRemnaConfigured, remnaGetUser, remnaGetUserByUsername, remnaGetUserByEmail, remnaGetUserByTelegramId, remnaGetUserByShortUuid, extractRemnaUuid, remnaUsernameFromClient, remnaGetUserHwidDevices, remnaDeleteUserHwidDevice, remnaDeleteUser, encryptSubscriptionUrlInPlace, remnaRevokeUserSubscription } from "../remna/remna.client.js";
...
/**
 * DELETE /api/client/subscription/secondary/:id
 * Клиент удаляет свою ИСТЁКШУЮ доп. подписку (root/основную удалить нельзя —
 * subscriptionIndex=0 отклоняется). Проверяем актуальный expireAt в Remnawave
 * (если Remna недоступна/не настроена — полагаемся на то, что фронт уже
 * показывает кнопку только у истёкших, но не даём удалить без этой проверки,
 * если данные удалось получить).
 */
clientRouter.delete("/subscription/secondary/:id", async (req, res) => {
  const clientId = (req as unknown as { clientId: string }).clientId;
  const subId = req.params.id;

  const sub = await prisma.subscription.findUnique({
    where: { id: subId },
    select: { id: true, ownerId: true, giftedToClientId: true, subscriptionIndex: true, remnawaveUuid: true },
  });
  if (!sub || (sub.ownerId !== clientId && sub.giftedToClientId !== clientId)) {
    return res.status(404).json({ message: "Подписка не найдена" });
  }
  if (sub.subscriptionIndex === 0) {
    return res.status(400).json({ message: "Основную подписку удалить нельзя" });
  }

  if (sub.remnawaveUuid && isRemnaConfigured()) {
    try {
      const r = await remnaGetUser(sub.remnawaveUuid);
      const payload = r.data as { expireAt?: string } | null;
      const expireAt = typeof payload?.expireAt === "string" ? payload.expireAt : null;
      if (expireAt && new Date(expireAt).getTime() > Date.now()) {
        return res.status(400).json({ message: "Нельзя удалить активную подписку" });
      }
    } catch {
      // Remna недоступна — не блокируем удаление локальной записи по этой причине.
    }
  }
  ...
  await prisma.giftHistory.create({
    data: {
      ...
      eventType: "DELETED",
      metadata: { deletedBy: "client" },
    },
  }).catch(() => { /* лог не критичен для успеха удаления */ });
