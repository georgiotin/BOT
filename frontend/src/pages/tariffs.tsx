                  ...
                  {tiers.map((t) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="h-7 w-7 rounded-md bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 flex items-center justify-center shrink-0 transition-colors"
                        title="Удалить порог"
                      >
    ...
    >
      ...
      {!isOnly && (
        <button
          type="button"
          onClick={onRemove}
          className="h-8 w-8 shrink-0 rounded-lg bg-foreground/[0.04] dark:bg-white/[0.04] border border-border text-muted-foreground hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-500/10 hover:border-blue-500/30 flex items-center justify-center transition-colors"
          title="Удалить опцию"
        >
          <X className="h-3.5 w-3.5" />
        </button>
    ...
    <div className="flex flex-col gap-3.5 relative">
      ...
      {error && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="rounded-xl border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm text-blue-500 dark:text-blue-400"
        >
          {error}
        </motion.div>
      )}
    ...
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="bg-card border-border rounded-2xl max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          {validationError && (
            <div className="flex items-start gap-2 rounded-xl border border-blue-500/30 bg-blue-500/10 px-3 py-2 text-xs text-blue-500 dark:text-blue-400">
              <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
