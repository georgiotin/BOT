...
export function RemnaNodesPage() {
  ...
  function KpiCard({ icon, label, value, sub, tone }: { icon: ReactNode; label: string; value: string; sub?: string; tone?: "emerald" | "red" | "primary" | "cyan" }) {
    const toneCls =
      ...
      : tone === "red" ? "bg-muted text-blue-500 dark:text-blue-400 border-blue-500/20"
    ...
    return (
      <button
        ...
        className={cn(
          ...
          danger ? "text-blue-500 dark:text-blue-400 hover:bg-blue-500/10" : "hover:bg-foreground/[0.06] dark:hover:bg-white/[0.06]"
        )}
      >
      ...
      {panelStats && (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-border bg-card px-4 py-3 flex items-center gap-x-5 gap-y-2 flex-wrap">
          ...
          {([
            ...
            ["EXPIRED", "истекло", "bg-blue-500/10 text-blue-500 dark:text-blue-400 border-blue-500/20"],
        ...
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-border bg-card px-4 py-3 flex items-center gap-x-5 gap-y-2 flex-wrap">
          ...
          {[...billingNodes]
            ...
            .map((b) => {
              ...
              const tone = days == null ? "" : days < 0 ? "bg-blue-500/10 text-blue-500 dark:text-blue-400 border-blue-500/20" : days <= 3 ? "bg-amber-500/10 text-amber-500 dark:text-amber-400 border-amber-500/20" : "bg-foreground/[0.04] dark:bg-white/[0.03] text-muted-foreground border-border";
              return (
              ...
              <motion.div key={n.uuid} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }} whileHover={{ y: -2 }}>
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    n.isDisabled ? "bg-zinc-500/60" : n.isConnected ? "bg-emerald-500" : "bg-blue-500"
                  )} />
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    {live ? (
                      ...
                      <div className="hidden lg:block text-xs text-blue-500/90 dark:text-blue-400/90 shrink-0 mr-1 max-w-[260px] truncate">
            ...
            <div className="space-y-4 py-2">
              ...
              {torrentReports.length > 0 && (
                <div className="flex justify-end">
                  <Button
                    variant="outline" size="sm" className="rounded-lg gap-1.5 text-xs text-blue-500 dark:text-blue-400 hover:bg-blue-500/10"
                    onClick={async () => {
                  ...
                  <div className="space-y-1.5">
                    {providers.map((pr) => (
                      <div key={pr.uuid} className="flex items-center justify-between gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 shrink-0" onClick={() => deleteProvider(pr.uuid, pr.name)}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    return (
                      <div key={b.uuid} className="flex items-center justify-between gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm">
                        ...
                        <span className="flex items-center gap-2 shrink-0">
                          <span className={cn("text-xs tabular-nums", days != null && days < 0 ? "text-blue-500 dark:text-blue-400" : days != null && days <= 3 ? "text-amber-500 dark:text-amber-400" : "text-muted-foreground")}>
                            {d?.toLocaleDateString("ru-RU", { day: "2-digit", month: "2-digit", year: "2-digit" })}{days != null && ` · ${days < 0 ? `−${-days}д` : `${days}д`}`}
                          </span>
                          <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg text-blue-500 dark:text-blue-400 hover:bg-blue-500/10" onClick={() => deleteBillingNode(b.uuid)}><Trash2 className="h-3.5 w-3.5" /></Button>
                        </span>
                      </div>
                    );
                  })}
            ...
            <div className="space-y-2 py-2">
              {plugins.map((pl) => (
                <div key={pl.uuid} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-foreground/[0.03] dark:bg-white/[0.02] px-3 py-2.5">
                  ...
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-blue-500 dark:text-blue-400 hover:bg-blue-500/10 shrink-0" title="Удалить плагин" onClick={() => deletePlugin(pl.uuid, pl.name)}>
