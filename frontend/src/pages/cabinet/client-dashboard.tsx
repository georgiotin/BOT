...
function ClassicDashboardPage() {
  ...
  // ♻️ FIX (мигание ползунков): раньше это был ОДИН string | null на весь список —
  // при быстром тапе по нескольким переключателям подряд они «отбирали» друг у друга
  // спиннер/disabled, из-за чего визуально дёргались (мигали) чужие строки.
  // Теперь у каждой подписки — свой независимый флаг «идёт запрос».
  const [togglingIds, setTogglingIds] = useState<Set<string>>(new Set());
  // ♻️ FIX: счётчик запросов по id — если пользователь тапнет один и тот же ползунок
  ...
  // FIX: независимый per-id toggling-флаг + защита от гонки устаревших ответов (см. autoRenewReqSeq).
  async function toggleSubAutoRenew(sub: { type: "root" | "secondary"; id: string }, enabled: boolean) {
    if (!token) return;
    const mySeq = (autoRenewReqSeq.current[sub.id] ?? 0) + 1;
    autoRenewReqSeq.current[sub.id] = mySeq;
    const isLatest = () => autoRenewReqSeq.current[sub.id] === mySeq;
    ...
    try {
      ...
      if (!isLatest()) return; // пришёл устаревший ответ — более новый тап уже всё решил
      refreshProfile().catch(() => {});
    } catch (err) {
      console.error("Failed to toggle subscription auto-renew", err);
      if (!isLatest()) return; // не откатываем поверх более нового состояния
  ...
  const autoRenewListNode = autoRenewSubs.length > 0 ? (
    <div className="relative overflow-hidden rounded-2xl border border-primary/15 bg-gradient-to-br from-primary/[0.07] via-background/40 to-primary/[0.06] backdrop-blur-xl p-4 text-left space-y-3 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)]">
        ...
        {autoRenewSubs.map((sub) => (
          ...
          >
            ...
            <span className="flex items-center gap-2 shrink-0">
              {/* FIX: фиксированная ширина слота под спиннер — раньше он монтировался/
                  размонтировался условно и «расталкивал» соседние элементы (визуально
                  выглядело как мигание строки при тапе по СОСЕДНЕМУ ползунку). */}
              <span className="flex h-3.5 w-3.5 items-center justify-center">
                {togglingIds.has(sub.id) && <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" />}
              </span>
              <Switch
                checked={sub.enabled}
  ...
  const NoSubscriptionState = () => (
    <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
      ...
      {rootActionNode(
        "gap-2 h-11 px-6 rounded-xl bg-primary text-primary-foreground border-0 shadow-lg shadow-primary/30 hover:opacity-90 [&_svg]:self-center [&_span]:leading-none",
        "h-4 w-4 shrink-0",
      ...
      <div className="w-full min-w-0 overflow-hidden flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <section data-tour="subscription" className="order-1 rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl p-5 shadow-sm overflow-hidden transition-all duration-300">
          <h2 className="flex items-center justify-between gap-2 text-[13px] font-semibold uppercase tracking-wider text-muted-foreground/80 mb-5">
            ...
            {rootActionNode(
              "shrink-0 gap-1.5 rounded-full h-8 px-3 bg-primary text-primary-foreground border-0 shadow-md shadow-primary/30 hover:opacity-90 normal-case [&_svg]:self-center [&_span]:leading-none",
            ...
            <div className="space-y-4 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                {hasActiveSubscription ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold bg-primary/20 text-primary dark:text-primary border border-primary/20">
                    <span className="h-1.5 w-1.5 rounded-full bg-current" />
                    Истекла
                  </span>
                )}
        ...
        {secondarySubscriptions.length > 0 && secondarySubscriptions.map((sec) => {
          ...
          return (
            <section key={sec.id} className="order-3 rounded-3xl border border-primary/30 bg-card/40 backdrop-blur-xl p-5 shadow-sm overflow-hidden transition-all duration-300">
              ...
              <div className="space-y-4 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  {secHasActive ? (
                    <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold bg-primary/20 text-primary border border-primary/20">
                      <span className="h-1.5 w-1.5 rounded-full bg-current" />
                      Активна
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold bg-primary/15 text-primary dark:text-primary border border-primary/20">
                      <span className="h-1.5 w-1.5 rounded-full bg-current" />
                      Истекла
                    </span>
                  )}
                ...
                <div className="space-y-3 border-t border-border/50 pt-4 mt-2">
                  ...
                  <div className="bg-background/40 p-3.5 rounded-2xl border border-border/50 space-y-3 transition-colors hover:bg-background/60 shadow-sm">
                    ...
                    {secTrafficPercent != null && (
                      <div className="h-2 w-full rounded-full bg-muted/30 overflow-hidden">
                        <div className="h-full rounded-full bg-primary transition-all duration-500 ease-in-out" style={{ width: `${secTrafficPercent}%` }} />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-2 flex flex-col sm:flex-row gap-2">
                  <Button className="flex-1 gap-2 shadow-lg h-12 rounded-xl text-md hover:scale-[1.02] transition-transform duration-300 [&_svg]:self-center [&_span]:leading-none bg-primary hover:bg-primary/90 text-primary-foreground" asChild>
        ...
        <section data-tour="balance" className="order-2 rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl p-5 shadow-sm overflow-hidden transition-all duration-300 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 p-2.5 pl-3 rounded-2xl bg-background/40 border border-border/50">
              ...
              {autoRenewPromoLoading ? (
                ...
                <button
                  ...
                  className="h-7 w-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-primary dark:hover:text-primary hover:bg-primary/10 transition-colors shrink-0"
          ...
          {anyAutoRenewOn && autoRenewPromoError && (
            <p className="text-[11px] font-medium text-primary dark:text-primary -mt-2">{autoRenewPromoError}</p>
          )}
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card data-tour="subscription" className="rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all duration-300 sm:col-span-2 lg:col-span-1 flex flex-col">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center justify-between gap-2 text-xl text-foreground">
              ...
              {rootActionNode(
                "shrink-0 gap-1.5 rounded-full h-9 px-4 bg-primary text-primary-foreground border-0 shadow-md shadow-primary/30 hover:opacity-90 hover:scale-105 transition-transform [&_svg]:self-center [&_span]:leading-none",
                "h-4 w-4 shrink-0",
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  {hasActiveSubscription ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[13px] font-semibold bg-primary/15 text-primary dark:text-primary border border-primary/20">
                      <span className="h-1.5 w-1.5 rounded-full bg-current" />
                      Истекла
                    </span>
                  )}
        ...
        <Card data-tour="balance" className="group relative overflow-hidden rounded-3xl border border-primary/15 bg-card/40 backdrop-blur-xl shadow-lg hover:shadow-xl hover:border-primary/30 transition-all duration-500 flex flex-col justify-between">
          ...
          <div className="pointer-events-none absolute -bottom-24 -left-16 h-48 w-48 rounded-full bg-primary/10 blur-3xl" aria-hidden />
          ...
          <CardHeader className="relative pb-4">
            <CardTitle className="flex items-center gap-3 text-xl text-foreground">
              <div className="p-2.5 rounded-xl bg-gradient-to-br from-primary/25 to-primary/20 ring-1 ring-primary/25 shadow-[0_0_20px_-6px] shadow-primary/40">
        ...
        <Card className="group relative overflow-hidden rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl shadow-lg hover:shadow-xl hover:border-primary/25 transition-all duration-500 sm:col-span-2 lg:col-span-1">
          {/* декоративные блобы */}
          <div className="pointer-events-none absolute -top-20 -left-16 h-48 w-48 rounded-full bg-primary/10 blur-3xl" aria-hidden />
              ...
              <>
                ...
                {referralStats && (
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { label: "Ваш %", value: `${referralStats.referralPercent}%`, icon: Percent, tint: "text-primary dark:text-violet-400", ring: "ring-primary/20", glow: "shadow-primary/25", bg: "from-primary/10" },
                    ].map((tile) => (
                ...
                <div className="rounded-2xl border border-border/40 bg-background/30 backdrop-blur-xl divide-y divide-border/40 overflow-hidden">
                  ...
                  {referralLinkBot && (
                    ...
                    >
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 ring-1 ring-primary/20">
                        <Send className="h-4 w-4 text-primary dark:text-violet-400" />
                      </span>
                      <span className="min-w-0 flex-1">
                      ...
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border/40 bg-background/40 text-foreground/60 group-hover/row:text-violet-400 group-hover/row:border-primary/30 group-hover/row:scale-105 transition-all">
                ...
                <div className="pt-1">
                  <Button variant="outline" className="group/btn w-full rounded-2xl h-12 text-[15px] font-medium bg-background/30 hover:bg-gradient-to-r hover:from-primary/10 hover:to-primary/10 hover:border-primary/30 transition-all duration-300 border-border/50 [&_svg]:self-center [&_span]:leading-none" asChild>
        ...
        >
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {secondarySubscriptions.map((sec) => {
              ...
              return (
                <Card key={sec.id} className="rounded-3xl border border-primary/30 bg-card/40 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col">
                  <CardHeader className="pb-4">
                    <CardTitle className="flex items-center justify-between gap-2 text-lg text-foreground">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {secHasActive ? (
                          <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[13px] font-semibold bg-primary/15 text-primary border border-primary/20">
                            <span className="h-1.5 w-1.5 rounded-full bg-current" />
                            Активна
                          </span>
                        ) : (
