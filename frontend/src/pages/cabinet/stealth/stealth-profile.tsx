...
import { StealthLanguageModal } from "@/components/stealth/stealth-language-modal";
...
export function StealthProfile() {
    ...
    const out: MenuSection[] = [
      {
        title: "Приложение",
        items: [
          { id: "language", label: "Язык", icon: Globe, rightLabel: langLabel, onClick: () => setShowLanguage(true) },
  ...
  return (
    <div className="px-4 pt-2 space-y-4 pb-2">
      ...
      >
        <div className="h-12 w-12 rounded-full bg-gradient-to-br from-saccent-600 to-saccent-400 flex items-center justify-center font-bold text-lg text-white shrink-0 shadow-[0_0_24px_-6px_rgb(var(--stealth-accent)_/_0.5)]">
        ...
        <div className="flex-1 min-w-0">
          ...
          {subInfo && (
            <div className="mt-1 inline-flex items-center gap-1.5 rounded-full bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 text-[10px] font-bold tracking-widest uppercase">
              <span className={cn("h-1.5 w-1.5 rounded-full", subInfo.active ? "bg-saccent-400 shadow-[0_0_6px_rgb(var(--stealth-accent)_/_0.7)]" : "bg-zinc-500")} />
      ...
      <StealthLanguageModal open={showLanguage} onClose={() => setShowLanguage(false)} />
    </div>
  );
}
