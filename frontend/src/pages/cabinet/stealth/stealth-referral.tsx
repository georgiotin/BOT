...
export function StealthReferral() {
  ...
  function share() {
    if (!link) return;
    // Внутри Telegram открываем родной диалог «Переслать» со списком чатов
    // вместо системного share.
  ...
  return (
    <div className="px-4 pt-2 space-y-4 pb-2">
      ...
      <button
        ...
        onClick={share}
        disabled={!link}
        className="w-full rounded-2xl border border-white/[0.08] bg-zinc-900/40 p-3 flex items-center gap-3 text-left hover:border-white/15 transition disabled:opacity-50 disabled:cursor-not-allowed"
      >
