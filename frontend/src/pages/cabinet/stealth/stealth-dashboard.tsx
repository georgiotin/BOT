...
interface SubCard {
  ...
  /** лимит устройств ЭТОЙ подписки (0/undefined → лимита нет либо неизвестен). */
  deviceLimit: number;
}
...
// T-fix-double-load (2026-08-13): раньше subs всегда стартовал с null, поэтому
// на каждом маунте (включая возврат на вкладку «Главная») карточка сперва
// показывала pulse-скелет и только потом настоящие данные — ощущалось как
// «подписка грузится дважды». Кэшируем последний успешный список в
// sessionStorage и используем его как стартовое состояние: карточка
// рендерится с прошлыми данными сразу, а свежие подтягиваются в фоне.
const SUBS_CACHE_KEY = "stealth_subs_cache_v1";

function readCachedSubs(): SubCard[] | null {
  try {
    const raw = sessionStorage.getItem(SUBS_CACHE_KEY);
    return raw ? (JSON.parse(raw) as SubCard[]) : null;
  } catch {
    return null;
  }
}
...
export function StealthDashboard() {
  ...
  const [subs, setSubs] = useState<SubCard[] | null>(() => readCachedSubs());
  const [devicesBySubId, setDevicesBySubId] = useState<Record<string, number>>({});
  const [loading, setLoading] = useState(subs === null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.getMyAllDevices(state.token).catch(() => ({ total: 0, items: [] })),
    ]).then(([all, allDev]) => {
      if (!alive) return;
      const devCounts: Record<string, number> = {};
      for (const d of (allDev.items || [])) devCounts[d.subscriptionId] = (devCounts[d.subscriptionId] || 0) + 1;
      const cards: SubCard[] = (all.items ?? []).map((it) => {
        ...
        const deviceLimit = Number.isFinite(limit) && limit > 0 ? limit : 0;
      ...
      try { sessionStorage.setItem(SUBS_CACHE_KEY, JSON.stringify(cards)); } catch { /* ignore */ }
  ...
  return (
    <div className="px-4 pt-2 space-y-5">
      ...
      <div className="relative h-44 md:h-56 flex items-center justify-center">
        ...
        >
          {heroImage ? (
            ...
            <Box className="h-14 w-14 md:h-16 md:w-16 text-saccent-500 drop-shadow-[0_0_12px_rgb(var(--stealth-accent)_/_0.6)]" strokeWidth={1.5} />
          )}
        </motion.div>
      </div>
      ...
      >
        ...
        {/* T-loading-anim (2026-08-13): пока список подписок ещё грузится —
            пульсирующий скелет вместо пустоты (subs===null — самый первый рендер). */}
        {loading && subs === null && (
          <div className="space-y-2.5" aria-hidden="true">
            {[0, 1].map((i) => (
              <div
                key={i}
                className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3.5 space-y-2.5 animate-pulse"
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <div className="h-7 w-7 rounded-xl bg-white/[0.05]" />
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-2">
                  ...
                  <div className="flex items-center gap-1.5 ml-auto min-w-0">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] hover:border-white/20 p-1.5 text-zinc-300 transition-all duration-300 active:scale-95 inline-flex items-center justify-center"
                    >
