  ...
  return (
    <div className="space-y-6 w-full min-w-0 pb-10">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {client.totpEnabled ? (
                    <>
                      ...
                      <Button variant="outline" size="sm" className="shadow-sm border-blue-500/50 text-blue-600 hover:bg-blue-500/15 dark:text-blue-400 dark:hover:bg-blue-500/20" onClick={openTwoFaDisable}>{t("cabinet.profile.2fa_disable")}</Button>
                    </>
                  ) : (
              ...
              {yookassaRecurringEnabled && client.yookassaPaymentMethodTitle && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-muted/40 border border-border/50 transition-colors hover:bg-muted/60 dark:bg-white/5 dark:border-white/5 dark:hover:bg-white/10">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="shadow-sm shrink-0 border-blue-500/50 text-blue-600 hover:bg-blue-500/15 dark:text-blue-400 dark:hover:bg-blue-500/20"
      ...
      <Dialog open={twoFaDisableOpen} onOpenChange={(open) => !open && setTwoFaDisableOpen(false)}>
        <DialogContent className="sm:max-w-md p-0 overflow-hidden border-border/50 backdrop-blur-3xl" showCloseButton={!twoFaLoading} onOpenAutoFocus={(e) => e.preventDefault()}>
          <div className="p-6 sm:p-8 flex flex-col items-center text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-blue-500/10 text-blue-500 mb-6 shadow-inner border border-blue-500/20">
            ...
            <div className="flex flex-col gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
              <Input
                ...
                value={twoFaCode}
                onChange={(e) => setTwoFaCode(e.target.value.replace(/\D/g, ""))}
                className="h-16 text-center text-3xl tracking-[0.3em] font-mono font-bold rounded-2xl border-blue-500/20 bg-blue-500/5 focus-visible:ring-blue-500/30"
                autoFocus
              />
              <Button variant="destructive" className="w-full h-12 rounded-2xl font-bold text-base shadow-lg shadow-blue-500/20" onClick={confirmTwoFaDisable} disabled={twoFaLoading || twoFaCode.length !== 6}>
                {twoFaLoading ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : null}
